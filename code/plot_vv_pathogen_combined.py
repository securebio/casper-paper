...
# =========================================================================
# Constants and helper functions ported from plot_pathogen_pmmov_panel.py
# =========================================================================

# Default sites to highlight in color (same as rrna norm panel)
...
from plot_config import MULTI_LAB_SITES, filter_timeseries_data

# Pathogens to plot: (taxid, label) - organized as 4x4 grid
# Row 1: Key respiratory pathogens
# Row 2: Common cold coronaviruses
# Row 3: Parainfluenza viruses
# Row 4: Gastrointestinal viruses
PATHOGENS = [
    # Row 1 - Key respiratory pathogens
    (2697049, 'SARS-CoV-2'),
    (197911, 'Influenza A'),
    ...
    (11137, 'HCoV-229E'),
    (31631, 'HCoV-OC43'),
    # Row 3 - Other respiratory viruses
    (2560525, 'Parainfluenza 2'),
    (11216, 'Parainfluenza 3'),
    (3048148, 'Metapneumovirus'),
...
def plot_pathogen_panel_on_axis(ax, mj_data, taxid, label, locations, highlight_sites,
                                 log_scale=True,
                                 normalize_by_site_median=False):
    """
    Plot PMMoV-normalized abundance time series for a single pathogen.

    Sites in highlight_sites are shown in color, others in gray.
    Multi-lab sites (MULTI_LAB_SITES) are split by sequencing_lab:
    MU data uses the site's highlight status, SB data is always gray.
    """
    taxid_data = mj_data[mj_data['taxid'] == taxid].copy()

    if len(taxid_data) == 0:
        ax.text(0.5, 0.5, f'No data for {label}', transform=ax.transAxes,
               ha='center', va='center', fontsize=FONT_SIZE_LARGE)
        ax.set_title(label, fontsize=FONT_SIZE_LARGE, weight='normal')
        return

    has_lab = 'sequencing_lab' in taxid_data.columns
    ...
    if normalize_by_site_median:
        ...
        def normalize_value(row):
            ...
            value = row['ra_clade_pmmov_norm']
            if site_name in site_medians and site_medians[site_name] > 0:
                return value / site_medians[site_name]
            return np.nan

        taxid_data['plot_value'] = taxid_data.apply(normalize_value, axis=1)
    else:
        taxid_data['plot_value'] = taxid_data['ra_clade_pmmov_norm']

    # Build list of (loc_data, is_highlighted) entries.
    # Multi-lab sites produce two entries: MU (may be highlighted) and SB (always gray).
    gray_items = []
    highlight_items = []

    for site_name in locations:
        site_data = taxid_data[taxid_data['site_name'] == site_name]
        is_multi = site_name in MULTI_LAB_SITES and has_lab
        ...
        if is_multi:
            for lab in ['MU', 'SB']:
                ...
                if len(lab_data) == 0:
                    continue
                if lab == 'MU' and site_name in highlight_sites:
                    highlight_items.append((site_name, lab_data))
                else:
                    gray_items.append((site_name, lab_data))
        else:
            loc_data = site_data.sort_values('date').reset_index(drop=True)
            if len(loc_data) == 0:
                continue
            if site_name in highlight_sites:
                highlight_items.append((site_name, loc_data))
            else:
                gray_items.append((site_name, loc_data))

    # Plot gray lines first (background)
    for site_name, loc_data in gray_items:
        if len(loc_data) >= 2:
            smoothed_df = calculate_mmwr_smoothed_trend(loc_data, 'date', 'plot_value')
            if not smoothed_df.empty:
                ax.plot(smoothed_df['date'], smoothed_df['smoothed_values'],
                       color='#CCCCCC', linewidth=LINE_WIDTH,
                       alpha=0.6, zorder=1)

    # Plot highlighted lines on top (foreground)
    for site_name, loc_data in highlight_items:
        color = get_location_color(site_name)

        ax.scatter(loc_data['date'], loc_data['plot_value'],
...
# =========================================================================
# Helper functions ported from plot_vv_family_composition_aggregated.py
# =========================================================================

def aggregate_vv_family_composition_for_site(merged_data, site_name, top_families):
    """
    Calculate read-count-weighted VV family composition for a site.
    """
    site_data = merged_data[merged_data['site_name'] == site_name].copy()

    if site_data.empty:
        return None

    site_data['family_group'] = site_data['name'].apply(
        lambda x: x if x in top_families else 'Other'
    )

    family_totals = site_data.groupby('family_group')['clade_counts'].sum().to_dict()

    total = sum(family_totals.values())
    proportions = {k: (v / total) if total > 0 else 0 for k, v in family_totals.items()}

    return proportions


def _plot_bars_on_axis(ax, locations, df, location_stats, title, top_families=None):
    """
    ...
    compositions = []
    for site_name in locations:
        comp = aggregate_vv_family_composition_for_site(df, site_name, top_families)
        if comp is None:
            comp = {fam: 0 for fam in families_with_other}
        compositions.append(comp)

    y_positions = range(len(locations))
    left = np.zeros(len(locations))

    for family in families_with_other:
        widths = [comp.get(family, 0) for comp in compositions]
        ax.barh(y_positions, widths, left=left,
               color=family_colors[family], edgecolor='none',
               height=0.8)
        left += widths
...
def _plot_boxplot_panel(ax, locations, vv_fractions, show_xlabel=True):
    """
    Plot horizontal boxplots showing VV fraction distribution.
    """
    df = vv_fractions.copy()
    ...
    for idx, site_name in enumerate(locations):
        loc_data = df[df['site_name'] == site_name]['vv_fraction'].values
        if len(loc_data) > 0:
    ...
    if show_xlabel:
        ax.set_xlabel('VV fraction', fontsize=FONT_SIZE_LARGE)
    else:
        ax.set_xlabel('')
...
def print_vv_fraction_statistics(vv_fractions):
    """Print VV fraction statistics for manuscript."""
    df = vv_fractions.copy()
    df = df[df['vv_fraction'].notna()]

    if df.empty:
        print("No VV fraction data available")
        return

    vv_frac = df['vv_fraction']
    median_vv = vv_frac.median()
    min_vv = vv_frac.min()
    max_vv = vv_frac.max()
    mean_vv = vv_frac.mean()

    print("\n" + "="*70)
    print("VV FRACTION STATISTICS")
    print("="*70)

    print(f"\nOverall (all samples):")
    print(f"  Count:     {len(vv_frac)}")
    print(f"  Median:    {100*median_vv:.4f}%")
    print(f"  Mean:      {100*mean_vv:.4f}%")
    print(f"  Range:     {100*min_vv:.4f}% - {100*max_vv:.4f}%")

    # By sequencing lab
    if 'sequencing_lab' in df.columns:
        for lab in sorted(df['sequencing_lab'].unique()):
            lab_data = df[df['sequencing_lab'] == lab]['vv_fraction']
            if len(lab_data) == 0:
                continue
            print(f"\n{lab}-sequenced samples:")
            print(f"  Count:     {len(lab_data)}")
            print(f"  Median:    {100*lab_data.median():.4f}%")
            print(f"  Mean:      {100*lab_data.mean():.4f}%")
            print(f"  Range:     {100*lab_data.min():.4f}% - {100*lab_data.max():.4f}%")

    print("\n" + "-"*70)
    print("FOR MANUSCRIPT:")
    print("-"*70)
    print(f"Across all samples, VV reads constitute a median of {100*median_vv:.2f}% of total "
          f"reads (range: {100*min_vv:.2f}%-{100*max_vv:.2f}%).")
    print("="*70 + "\n")
...
def plot_vv_pathogen_combined(save_path=None, log_scale=False,
    ...
    df_vv = vv_per_library.merge(
        virus_data[['site_name', 'sra_accession', 'total_read_pairs']],
    ...
    df_vv = df_vv.merge(meta[['sra_accession', 'sequencing_lab']].drop_duplicates(),
    ...
    # Add sequencing_lab to vv_fractions for filtering
    ...
    if log_scale:
        for row_idx in range(N_ROWS):
            ...
            if not row_data.empty:
                ...
                for site_name in pathogen_locations:
                    ...
                    if is_multi:
                        # Split by lab, matching plot_pathogen_panel_on_axis behavior
                        for lab in ['MU', 'SB']:
                            lab_data = site_data[site_data['sequencing_lab'] == lab].sort_values('date').reset_index(drop=True)
                            if len(lab_data) == 0:
                                continue
        ...
        for idx, (taxid, label) in enumerate(PATHOGENS):
            ...
            def normalize_value(row):
                sn = row['site_name']
                if sn in site_medians and site_medians[sn] > 0:
                    return row['ra_clade_pmmov_norm'] / site_medians[sn]
                return np.nan
    ...
    for h, label in zip(handles, labels):
        if 'Miami' in label or 'FL' in label:
    ...
    ylabel_x = leftmost_panel_x - 0.05
    if normalize_by_site_median:
