#!/usr/bin/env python3
"""
Time series of median quality score per sample for ALL sites (supplementary figure).

Creates line/scatter plots showing median per-sequence Phred quality score over time
for all sites, arranged in 2 columns. Sites are ordered by sequencing lab (MU first,
then SB), then by state, then alphabetically within state.

Uses raw (pre-trimming) quality scores as this reflects the quality of data deposited
in SRA, which users will download and process with their own pipelines.
"""
...
from data_loaders import load_all_site_data, load_all_quality_data
...
def plot_quality_score_timeseries_all(quality_data, site_data, save_path=None):
    """
    Create time series scatter/line plots of median quality score for all sites.

    Sites are arranged in 2 columns, with MU-sequenced sites first (with gray header),
    then SB-sequenced sites (with gray header).

    Args:
        quality_data: DataFrame from load_all_quality_data()
    ...
    """
    # Merge quality data with site data to get dates and sequencing_lab
    merge_cols = ['site_name']
    if 'sra_accession' in quality_data.columns and 'sra_accession' in site_data.columns:
    ...
    q = merged['median_quality_score']
    print(f"  Overall raw median Q score: median={q.median():.1f}  "
          f"mean={q.mean():.1f}  range=[{q.min():.1f}, {q.max():.1f}]")
    for lab in ['MU', 'SB']:
        lab_q = merged.loc[merged['sequencing_lab'] == lab, 'median_quality_score']
        if len(lab_q) > 0:
            print(f"  {lab}-sequenced (n={len(lab_q)}): median={lab_q.median():.1f}  "
                  f"mean={lab_q.mean():.1f}  range=[{lab_q.min():.1f}, {lab_q.max():.1f}]")
    ...
    fig.text(0.02, 0.5, 'Median quality score (Phred)', va='center', rotation='vertical',
             fontsize=FONT_SIZE_LARGE)
...
def _plot_sites_on_axes(axes, sites, merged_data, date_min, date_max, date_range_days,
    ...
    """
    Helper function to plot quality scores on a set of axes.

    Args:
        axes: List of matplotlib axes
        sites: List of site names to plot
        merged_data: DataFrame with quality score and date data
    ...
    # Fixed y-axis range for quality scores (Phred scores typically 30-40 for good data)
    y_min = 30
    y_max = 42
    ...
    for idx, site_name in enumerate(sites):
        ...
        dates = loc_data['date']
        quality_scores = loc_data['median_quality_score']

        # Get color for this site
        color = get_location_color(site_name)

        # Create scatter plot with line
        ax.plot(dates, quality_scores, color=color, linewidth=1.5, alpha=0.6, zorder=1)
        ax.scatter(dates, quality_scores, color=color, s=25, alpha=0.8, edgecolors='none', zorder=2)

        # Format subplot
        ax.set_ylim(y_min, y_max)
        ax.set_yticks([32, 36, 40])
        ax.set_yticklabels(['32', '36', '40'])
