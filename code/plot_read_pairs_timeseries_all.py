#!/usr/bin/env python3
"""
Time series of read pairs per sample for ALL sites (supplementary figure).

Creates bar charts showing read pairs over time for all sites, arranged in
...
from data_loaders import load_all_site_data
...
def plot_read_pairs_timeseries_all(site_data, save_path=None):
    """
    Create time series bar charts of read pairs for all sites.
    ...
    """
    # Get ordered sites
    mu_sites, sb_sites = get_ordered_sites(site_data)
    ...
    mu_data = site_data[site_data['sequencing_lab'] == 'MU']
    sb_data = site_data[site_data['sequencing_lab'] == 'SB']
    ...
    if len(axes_mu) > 0:
        ...
        # MU header spans both columns - position higher to not cover titles
    ...
    if len(axes_sb) > 0:
        ...
        # SB header spans both columns - position higher to not cover titles
    ...
    fig.text(0.02, 0.5, 'Read pairs (billions)', va='center', rotation='vertical',
             fontsize=FONT_SIZE_LARGE)
...
def _plot_sites_on_axes(axes, sites, site_data, date_min, date_max, date_range_days,
                        ncols, nrows, show_date_labels=True):
    """
    Helper function to plot read pairs on a set of axes.

    Args:
        axes: List of matplotlib axes
        sites: List of site names to plot
        site_data: DataFrame with read pairs data
    ...
    # Get global max for y-axis scaling across all sites
    global_max = site_data['total_read_pairs'].max() / 1e9  # Convert to billions

    for idx, site_name in enumerate(sites):
        ax = axes[idx]
        loc_data = site_data[site_data['site_name'] == site_name].sort_values('date')
        ...
        dates = loc_data['date']
        read_pairs = loc_data['total_read_pairs'] / 1e9  # Convert to billions
        ...
        # Create bars
        ax.bar(dates, read_pairs, width=bar_width, color=color, edgecolor='none', alpha=0.8)
    ...
    for idx in range(len(sites), len(axes)):
        ax = axes[idx]
        # For empty cells in the bottom row, keep them visible but empty with date labels
        row = idx // ncols
        if row == nrows - 1 and show_date_labels:
            # Keep this axis visible but with no data, just date labels
            ax.set_xlim(date_min - buffer, date_max + buffer)
            ax.set_ylim(0, global_max * 1.05)
            ax.set_yticks([])
            ax.spines['top'].set_visible(False)
